...
**Firebase** est un service **gratuit de Google** (plan *Spark*, 0 €) qui va :
- garder le contenu de votre site en ligne (textes + photos),
...
1. Menu de gauche → **Databases & Storage → « Firestore Database »** →
   **« Créer une base de données »**.
2. Emplacement : `eur3 (europe-west)` → **Suivant**.
3. Démarrez en **« mode production »** → **Créer**.
4. Onglet **« Règles »**. Effacez tout et collez ceci, en remplaçant l'e-mail
   par le(s) vôtre(s) — **la même adresse que celle avec laquelle vous vous
   connecterez** :
  ...
  match /databases/{database}/documents {
    match /site/content {
      ...
      allow write: if request.auth != null
        && request.auth.token.email in ["orecarole54@gmail.com", "vicab2002@gmail.com"];
    }
  }
}
```
...
> **Storage n'est pas utilisé.** Il est devenu payant sur les nouveaux projets
> Firebase. Les **textes** et les **photos** (compressées automatiquement) se
> modifient en ligne via Firestore. Pour changer une **vidéo**, on remplace le
> fichier dans `assets/` (même nom) puis on redéploie — c'est rare.

### 2.5 — Autoriser votre adresse de site
...
### 2.6 — Mettre les clés en ligne

`firebase-config.js` a changé. Si le site est relié à GitHub (cas Netlify via
l'agent) : ouvrez `https://github.com/<votre-compte>/lafleuristes/blob/main/firebase-config.js`
→ crayon **Edit** → collez vos valeurs → **Commit changes**. Netlify redéploie
seul (~15 s).
Sinon : **re-glissez le dossier** sur Netlify. C'est la dernière fois pour du contenu.
...
même adresse dans les **règles Firestore** (Partie 2.4), sinon la publication
sera refusée.
